...
class SocialChef_Theme_Utils {
	...
	public static function breadcrumbs() {
		...
		$breadcrumbs_html = ob_get_contents();
		ob_end_clean();
		
		$breadcrumbs_html = apply_filters('socialchef_breadcrumbs', $breadcrumbs_html);
		
		echo $breadcrumbs_html;
	}
		
	public static function display_pager($max_num_pages, $pag_arg = 'paged-sc') {
	
		$pattern = '#(www\.|https?:\/\/){1}[a-zA-Z0-9]{2,254}\.[a-zA-Z0-9]{2,4}[a-zA-Z0-9.?&=_/]*#i';

		$page_arg = sanitize_key( $pag_arg );
		...
		if ($page_arg == 'paged-sc') {
			...
			$format = '';
			$base = add_query_arg( $page_args, $base );
			$current = get_query_var($pag_arg);
		}
		
		// $pager_links = paginate_links( array(
			// 'base'      => $pag_arg == 'paged-sc' ? str_replace( $big, '%#%', esc_url( get_pagenum_link( $big ) ) ) : add_query_arg( $page_args, $base ),
			// 'format' => $pag_arg == 'paged-sc' ? '?paged-sc=%#%' : '',
			// 'current' => max( 1, get_query_var($page_arg) ),
			// 'total' => $max_num_pages,
		...
		$uri = SocialChef_Theme_Utils::get_current_page_url();
		$urls = explode( '?', esc_url_raw( add_query_arg( array(), $uri ) ) );
		$uri = $urls[0];
		$first_uri = add_query_arg( 'mrp', 1, $uri );
		$last_uri = add_query_arg( 'mrp', $max_num_pages, $uri );
		
		$count_links = count($pager_links);
		if ($count_links > 0) {
			echo '<span><a href="' . esc_url( $first_uri ) . '">' . __('&laquo;', 'socialchef') . '</a></span>';
			...
			echo '<span><a href="' . esc_url( $last_uri ) . '">' . __('&raquo;', 'socialchef') . '</a></span>';
		}
	}
