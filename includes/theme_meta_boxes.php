...
class SocialChef_Theme_MetaBoxes extends SocialChef_BaseSingleton {
	...
	public function setup_meta_boxes() {
		...
		$this->recipe_list_custom_meta_fields = array(
			...
			array( // Taxonomy Select box
				'label'	=> __('Category', 'socialchef'), // <label>
				// the description is created in the callback function with a link to Manage the taxonomy terms
				'id'	=> 'recipe_category', // field id and name, needs to be the exact name of the taxonomy
				'type'	=> 'tax_select' // type of field
			),			
			array( // Select box
		...
		if ($sc_theme_globals->enable_instructions()) {		
		
			$instructions = array( // Repeatable & Sortable Text inputs
			...
			if ($sc_theme_globals->enable_instruction_images()) {		
				$instructions['repeatable_fields'][] =
			...
			$this->recipe_custom_meta_fields[] = $instructions;
		}
		
		if ($sc_theme_globals->enable_recipe_meta()) {			
			$this->recipe_custom_meta_fields[] = array( // Taxonomy Select box
		...
		if ($sc_theme_globals->enable_ingredients()) {		
