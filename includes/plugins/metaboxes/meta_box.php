...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		break;
		case 'url':
			echo '<input type="' . esc_attr ( $type ) . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . esc_url( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		case 'number':
			echo '<input type="' . esc_attr ( $type ) . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . intval( $meta ) . '" class="regular-text" size="30" />
					<br />' . $desc;
		break;
		// textarea
		case 'textarea':
		...
		case 'post_chosen':
			...
			echo '</select> &nbsp;<span class="description"><a href="' . esc_url (admin_url( 'edit.php?post_type=' . $post_type[0] . '">Manage ' . $post_type_object->labels->name ) ) . '</a></span><br />' . $desc;
		break;
		// post_checkboxes
		case 'post_checkboxes':
			...
			$post_type_object = get_post_type_object( $post_type[0] );
			echo '</ul> ' . $desc , ' &nbsp;<span class="description"><a href="' . esc_url ( admin_url( 'edit.php?post_type=' . $post_type[0] . '">Manage ' . $post_type_object->labels->name ) ) . '</a></span>';
		break;
		// post_drop_sort
		case 'post_drop_sort':
			//areas
			$post_type_object = get_post_type_object( $post_type[0] );
			echo '<p>' . $desc . ' &nbsp;<span class="description"><a href="' . esc_url ( admin_url( 'edit.php?post_type=' . $post_type[0] . '">Manage ' . $post_type_object->labels->name ) ) . '</a></span></p><div class="post_drop_sort_areas">';
		...
		case 'tax_select':
			...
			foreach ( $terms as $term ) {
				...
				if (!$repeatable)
					...
					$term_term = get_term_by('slug', $meta, $taxonomy->name);
					if ($term_term)
						$selected = $term_term->slug;
				}
			...
			echo '</select> &nbsp;<span class="description"><a href="'. esc_url ( home_url() ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		break;
		// tax_checkboxes
		case 'tax_checkboxes':
			...
			echo '<span class="description">' . (isset($field['desc']) ? $field['desc'] : '') . ' <a href="'. esc_url ( home_url() ) . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>';
		break;
		// date
		case 'date':
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					do_action('meta_box_delete_post_meta', $post_id, $field);
					...
					do_action('meta_box_update_post_meta', $post_id, $field, $new);
				}
			}
		} // end foreach
	}
