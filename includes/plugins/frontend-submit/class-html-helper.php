...
class Html_Helper {
	...
	function checkboxes( $name = '', $description = '', $data = array(), $checked = array() ) {
		if ( $name != '' ) {
			...
			if ( $description );
			echo $this->element( 'p', $description );
	 ...
	 * @param mixed   $data
	 */
	function input( $type, $name, $data = null, $attrs = array(), $description = '' ) {
		switch ( $type ) {
		case 'select':
			return $this->_select( $name, $data, $attrs );
			break;
		case 'text':
			return $this->_text( $name, $type,  $data, $attrs, $description ) ;
			break;
		case 'hidden':
		case 'submit':
		case 'file':
	 ...
	 * @access private
	 */
	function _text( $name = '', $type='text', $data = '', $attrs = array(), $description = '' ) {
		if ($type == 'text' && empty($data))
			return '<input type="' . esc_attr( $type ) . '" placeholder="'. esc_attr( $description ) . '" name="' . esc_attr( $name ) . '" '.$this->_format_attributes( $attrs ) . ' />';
		else
	...
	/**
	 * Renders html element start tag
	 *
	 * @param string  $tag    one of allowed tags
	 * @param array   $params additional attributes
	 * @return string rendered html tag
	 */
	function element_start( $tag, $params = array() ) {
		$allowed = apply_filters( 'hh_allowed_html_elements' , array( 'div', 'p', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'td', 'option', 'label', 'textarea', 'select', 'option' ) );
		$attr_string = $this->_format_attributes( $params );
		if ( in_array( $tag, $allowed ) )
	...
	/**
	 * Renders html element end tag
	 *
	 * @param string  $tag    one of allowed tags
	 * @param array   $params additional attributes
	 * @return string rendered html tag
	 */
	function element_end( $tag ) {
		$allowed = apply_filters( 'hh_allowed_html_elements' , array( 'div', 'p', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'td', 'option', 'label', 'textarea', 'select', 'option' ) );
		if ( in_array( $tag, $allowed ) )
			return "</{$tag}>";
	}
