...
class SocialChef_Recipes_Post_Type extends SocialChef_BaseSingleton {
	...
	private $enable_ingredients;
	private $enable_recipe_meta;

	protected function __construct() {
	
		$this->enable_nutritional_elements = of_get_option('enable_nutritional_elements', 1);
		$this->enable_ingredients = of_get_option('enable_ingredients', 1);
		$this->enable_recipe_meta = of_get_option('enable_recipe_meta', 1);
	...
	function create_post_type_recipe() {
		...
		global $sc_theme_globals, $wp_rewrite;
		$wp_rewrite->use_verbose_page_rules = true;
		
		$recipes_permalink_slug = $sc_theme_globals->get_recipes_permalink_slug();
		
		$recipe_list_page_id = $sc_theme_globals->get_recipe_list_page_id();
		
		if ($recipe_list_page_id > 0) {

			add_rewrite_rule(
				"{$recipes_permalink_slug}$",
				"index.php?post_type=page&page_id={$recipe_list_page_id}", 'top');
		
			add_rewrite_rule(
				"{$recipes_permalink_slug}/page/?([1-9][0-9]*)",
				"index.php?post_type=page&page_id={$recipe_list_page_id}&paged=\$matches[1]", 'top');		
		}
		
		add_rewrite_rule(
			"{$recipes_permalink_slug}/([^/]+)/page/?([1-9][0-9]*)",
			"index.php?post_type=recipe&name=\$matches[1]&paged-sc=\$matches[2]", 'top');
			
		add_rewrite_tag('%paged-sc%', '([1-9][0-9]*)');	
		add_rewrite_tag('%frp%', '([1-9][0-9]*)');	
		add_rewrite_tag('%mrp%', '([1-9][0-9]*)');			

		$labels = array(
			'name'                => __( 'Recipes', 'socialchef' ),
			'singular_name'       => __( 'Recipe', 'socialchef' ),
		...
		$args = apply_filters('filter_create_post_type_recipe', $args);
	...
	function create_meal_course_taxonomy(){
		...
		$labels = array(
				'name'              			=> __( 'Meal course', 'socialchef' ),
				'singular_name'     			=> __( 'Meal course', 'socialchef' ),
		...
		$args = apply_filters('filter_create_meal_course_taxonomy', $args);
	...
	function create_recipe_category_taxonomy() {
		...
		$labels = array(
				'name'              			=> __( 'Categories', 'socialchef' ),
				'singular_name'     			=> __( 'Category', 'socialchef' ),
		...
		$args = apply_filters('filter_create_recipe_category_taxonomy', $args);
	...
	function create_recipe_difficulty_taxonomy(){
		...
		$labels = array(
				'name'              			=> __( 'Difficulty', 'socialchef' ),
				'singular_name'     			=> __( 'Difficulty', 'socialchef' ),
		...
		$args = apply_filters('filter_create_recipe_difficulty_taxonomy', $args);
	...
	function create_ingredient_taxonomy() {
		...
		$labels = array(
				'name'              			=> __( 'Ingredients', 'socialchef' ),
				'singular_name'     			=> __( 'Ingredient', 'socialchef' ),
		...
		$args = apply_filters('filter_create_ingredient_taxonomy', $args);
	...
	function create_ingredient_unit_taxonomy(){
		...
		$labels = array(
				'name'              			=> __( 'Ingredient Units', 'socialchef' ),
				'singular_name'     			=> __( 'Ingredient Unit', 'socialchef' ),
		...
		$args = apply_filters('filter_create_ingredient_unit_taxonomy', $args);
	...
	function create_nutritional_element_taxonomy() {
		...
		$labels = array(
				'name'              			=> __( 'Nutritional elements', 'socialchef' ),
				'singular_name'     			=> __( 'Nutritional element', 'socialchef' ),
		...
		$args = apply_filters('filter_create_nutritional_element_taxonomy', $args);
	...
	function create_nutritional_unit_taxonomy(){
		...
		$labels = array(
				'name'              			=> __( 'Nutritional Units', 'socialchef' ),
				'singular_name'     			=> __( 'Nutritional Unit', 'socialchef' ),
		...
		$args = apply_filters('filter_create_nutritional_unit_taxonomy', $args);
