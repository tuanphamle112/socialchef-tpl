...
class SocialChef_Theme_Actions extends SocialChef_BaseSingleton {
    ...
    public function init() {
		...
		// add_action( 'bp_enqueue_scripts', array($this, 'enqueue_scripts' ), 1 );
	...
	function bp_setup_nav() {
		...
		$my_recipes_slug = 'my-recipes';
		$fav_recipes_slug = 'favorite-recipes';
		...
		bp_core_new_nav_item( array( 
			...
			'default_subnav_slug' => $my_recipes_slug
		) );
	...
	function bp_my_recipes_tab_screen() {
		...
		bp_core_load_template( 'recipes/my-recipes' ) ;
	}
	
	function bp_favorite_recipes_tab_screen() {
		global $bp;
		do_action( 'bp_socialchef_favorite_recipes_screen' );
		bp_core_load_template( 'recipes/favorite-recipes' );
	}
	...
	function setup() {
		...
		register_sidebar(array(
			...
			'after_widget' => '',
			'class'	=> '',
			'before_title' => '<div class="ribbon bright"><div><h2>',
	...
	function enqueue_scripts() {
		...
		if ( is_home() || is_front_page() || $template_file == 'sc_home.php' ) {
		...
		wp_enqueue_script('fontawesome', 'https://use.fontawesome.com/e808bf9397.js', array(), '1.0', "screen");
		...
		$predecessor_stylesheets = array();
		
		if (SocialChef_Theme_Utils::is_buddypress_active()) {
			if (!is_rtl()) {
				$predecessor_stylesheets[] = 'bp-parent-css';
			} else {
				$predecessor_stylesheets[] = 'bp-legacy-css-rtl';			
			}
		}
		
		if (SocialChef_Theme_Utils::is_bbpress_active()) {
			if (!is_rtl()) {
				$predecessor_stylesheets[] = 'bbp-default';
			} else {
				$predecessor_stylesheets[] = 'bbp-default-rtl';
			}
		}
		
		if (SocialChef_Theme_Utils::is_woocommerce_active()) {
			$predecessor_stylesheets[] = 'woocommerce-smallscreen';
			$predecessor_stylesheets[] = 'woocommerce-layout';
			$predecessor_stylesheets[] = 'woocommerce-general';
		}
		...
		if ($sc_theme_globals->enable_rtl() || (defined('SC_DEMO') && isset($_REQUEST['rtl']))) {
			wp_enqueue_style( 'socialchef-style-main', SocialChef_Theme_Utils::get_file_uri ('/css/style-rtl.css'), $predecessor_stylesheets, '1.0', "screen,projection,print");
		} else {
			wp_enqueue_style('socialchef-style-main',  SocialChef_Theme_Utils::get_file_uri('/css/style.css'), $predecessor_stylesheets, '1.0', "screen,projection,print");
		}
		
		wp_enqueue_style( 'socialchef-style', get_stylesheet_uri(), array('socialchef-style-main') );		
		...
		if (isset($post)) {
			...
			if ($template_file == 'page-user-submit-recipe.php') {
				...
				wp_enqueue_style( 'socialchef-dropzone-style', SocialChef_Theme_Utils::get_file_uri('/includes/plugins/frontend-submit/dropzone.min.css') );
				wp_enqueue_script( 'socialchef-dropzone', SocialChef_Theme_Utils::get_file_uri ('/includes/plugins/frontend-submit/dropzone.min.js'), array( 'jquery', 'socialchef-jquery-validate' ), '1.0', true );
	...
	function register_google_fonts() {
		...
		wp_register_style('Raleway', '//fonts.googleapis.com/css?family=Raleway:400,300,500,600,700,800');
