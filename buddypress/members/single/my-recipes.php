...
} else if ( get_query_var('mrp') ) {
    $paged = get_query_var('mrp');
} else {
    $paged = 1;
}
...
$show_private_recipes = false;
if ($displayed_user_id > 0 && $current_user->ID == $displayed_user_id) {
	$show_private_recipes = true;
}

$recipe_results = $sc_recipes_post_type->list_recipes($paged, $posts_per_page, '', '', array(), array(), array(), array(), array(), false, $displayed_user_id, $show_private_recipes); 
...
<div class="pager">
	...
	SocialChef_Theme_Utils::display_pager( ceil($total_results/$posts_per_page), 'mrp' ); 
	?>
</div>
