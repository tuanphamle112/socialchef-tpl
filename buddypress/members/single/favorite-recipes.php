...
} else if ( get_query_var('frp') ) {
    $paged = get_query_var('frp');	
} else {
    $paged = 1;
}
...
if (empty($favorite_ids)) {
	$favorite_ids = array(-1);
}

$recipe_results = $sc_recipes_post_type->list_recipes($paged, $posts_per_page, '', '', array(), array(), array(), array(), array(), false, null, false, $favorite_ids); 
...
<div class="pager">
	...
	SocialChef_Theme_Utils::display_pager( ceil($total_results/$posts_per_page), 'frp' ); 
	?>
</div>
